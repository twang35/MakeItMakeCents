...
class TechTimeEnv(gym.Env):
    ...
    def __init__(
        ...
        self.i = TechTimeState.context_window
    ...
    def step(self, action):
        ...
        return (state, step_reward, terminated, truncated,
                {'stonks_state': TechTimeState(state, self.get_total_balance(),
    ...
    def show_price_map(self):
        fig = make_subplots()
        fig.update_layout(title=dict(text=f'price', font=dict(size=25)))

        scaled_prices = self.price_scaler.transform(np.array(self.token_prices.data).reshape(-1, 1))
        scaled_prices = scaled_prices.reshape(1, -1)[0]
        fig.add_trace(go.Scatter(x=self.token_prices.timestamps, y=scaled_prices, name='hourly prices'))
